...
async function syncThemeFromAccount(member) {
    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            if (typeof prefs.serena_voice_enabled === "boolean") {
                setSerenaVoiceEnabled(prefs.serena_voice_enabled);
            }
...
// AIアシスタント「セレナ」
// =================================
// 20代半ばの落ち着いた女性という設定：優しい・知的・冷静、必要な時だけ少し感情が乗る口調
//
// 現在はルールベース(キーワード一致)で応答している。
// 将来 本物のAI APIに切り替える場合は、serenaGetReply() の中身を
// fetch(...) 等に差し替えるだけで済むよう、非同期関数として設計している。

const SERENA_VOICE_LANG = "ja-JP";

// イベントに応じた定型の音声ガイド
const SERENA_EVENT_LINES = {
    login: "お帰りなさい。",
    error: "エラーが発生しました。",
    mail: "新着メールがあります。",
    task_due: "期限が近いタスクがあります。"
};

// セレナの音声ガイド(読み上げ)のON/OFF設定
// OFFにしても、チャット欄への文字表示や吹き出しは引き続き行われる(音声のみ止める)
export function getSerenaVoiceEnabled() {
    return localStorage.getItem("ARG_SERENA_VOICE") !== "0"; // 未設定時はデフォルトON
}

export function setSerenaVoiceEnabled(enabled) {
    localStorage.setItem("ARG_SERENA_VOICE", enabled ? "1" : "0");
}

function serenaSpeak(text) {

    if (!("speechSynthesis" in window)) return;
    if (!getSerenaVoiceEnabled()) return; // 音声ガイドがOFFの場合は読み上げない

    try {

        window.speechSynthesis.cancel(); // 直前の発話が残っていれば止めてから話す

        const utterance = new SpeechSynthesisUtterance(text);
        utterance.lang = SERENA_VOICE_LANG;
        utterance.rate = 1.0;
        utterance.pitch = 1.05;

        // 日本語の女性ボイスがあれば優先する(対応状況はブラウザ/OS依存)
        const voices = window.speechSynthesis.getVoices();
        const jaVoice =
            voices.find((v) => v.lang && v.lang.startsWith("ja") && /female|女性/i.test(v.name)) ||
...
function serenaShowBubble(text) {

    let bubble = document.getElementById("serenaBubble");

    if (!bubble) {
        bubble = document.createElement("div");
        bubble.id = "serenaBubble";
        bubble.className = "serena-bubble";
    ...
    clearTimeout(window.__serenaBubbleTimeout);
    window.__serenaBubbleTimeout = setTimeout(() => bubble.classList.remove("active"), 6000);

}

// extra: { link_url, link_label, choices: [{label, value}] } を渡すと、
// 返信テキストの下にリンクや選択肢ボタンを表示できる
function serenaLogLine(who, text, extra) {

    const log = document.getElementById("serenaLog");
    if (!log) return;

    const line = document.createElement("div");
    line.className = "serena-line serena-line-" + who;

    const textEl = document.createElement("div");
    textEl.textContent = (who === "serena" ? "🌸 " : "🧑 ") + text;
    line.appendChild(textEl);

    if (extra && extra.link_url) {

        const link = document.createElement("a");
        link.href = extra.link_url;
        link.textContent = extra.link_label || "詳しくはこちら";
        link.className = "serena-reply-link";

        // http(s)から始まる外部リンクは新しいタブで開く。サイト内のページは同じタブで移動する
        if (/^https?:\/\//i.test(extra.link_url)) {
            link.target = "_blank";
            link.rel = "noopener";
        }
    ...
    if (extra && Array.isArray(extra.choices) && extra.choices.length > 0) {

        const choiceRow = document.createElement("div");
        choiceRow.className = "serena-choice-row";

        extra.choices.forEach((choice) => {

            const btn = document.createElement("button");
            btn.type = "button";
            btn.className = "serena-choice-btn";
            btn.textContent = choice.label;

            btn.onclick = () => {
                document.dispatchEvent(new CustomEvent("serena:choice", { detail: choice.value }));
            };
...
// イベント音声ガイド（ログイン・エラー・新着メール・タスク期限 等）
// customText を渡さない場合は SERENA_EVENT_LINES から定型文を使う
export function serenaAnnounce(eventKey, customText) {

    const text = customText || SERENA_EVENT_LINES[eventKey];
    if (!text) return;

    serenaSpeak(text);
    serenaShowBubble(text);
    serenaLogLine("serena", text);

}

// 初期状態で用意している既定の応答ルール（管理画面から追加したものが優先して照合される）
const DEFAULT_SERENA_RULES = [
    { keywords: ["ログイン", "login"], reply: "ログインには、登録済みのMIDとパスワードが必要です。MIDは「ARG-」に続く5桁の数字部分だけを入力してください。" },
    { keywords: ["パスワード", "password"], reply: "パスワードの変更は、設定画面の個人設定から行えます。現在のパスワードの入力が必要ですので、あらかじめご確認くださいね。" },
    { keywords: ["タスク", "task"], reply: "タスクの管理はTASKS画面から行えます。カレンダーの「＋ NEW TASK」から新しいタスクを作成できますよ。" },
    { keywords: ["メール", "mail"], reply: "MAIL画面でメールの確認や送信ができます。新着メールがあると、ダッシュボードのNEWSにも表示されます。" },
    { keywords: ["企画", "プロジェクト", "project"], reply: "PROJECTSでは企画の投稿・編集・削除ができます。編集や削除は、ご自身の投稿か管理者の方のみ行えます。" },
    { keywords: ["スレッド", "records", "thread"], reply: "THREADSではスレッドを立てたり、書き込みをしたりできます。リアクションのスタンプも豊富にご用意していますよ。" },
    { keywords: ["ファイル", "file"], reply: "FILES機能は現在準備中です。もう少しだけお待ちくださいね。" },
    { keywords: ["設定", "テーマ", "settings"], reply: "SETTINGSでは表示名やパスワードの変更のほか、カラーテーマやフォントの変更もできます。ぜひお好みの見た目を探してみてください。" },
    { keywords: ["権限", "アクセスレベル", "level"], reply: "権限レベルは1から5まであり、数字が大きいほど強い権限になります。詳しくは管理者の方にご確認いただけますでしょうか。" },
    { keywords: ["セレナ", "あなたは誰", "自己紹介"], reply: "はじめまして、セレナと申します。ARG Controlのご利用をサポートさせていただきますね。" },
    { keywords: ["ありがとう", "thanks"], reply: "どういたしまして。お役に立てて嬉しいです。" },
    { keywords: ["こんにちは", "hello", "hi", "やあ"], reply: "こんにちは。何かお手伝いできることはありますか？" }
];

// 管理画面(serena-rules.html)から登録された応答ルールのキャッシュ
// ページ読み込み時に一度だけFirebaseから取得し、以降はこの配列を使い回す
let customSerenaRules = null;

async function loadCustomSerenaRules() {

    if (customSerenaRules !== null) return customSerenaRules;

    try {

        const snapshot = await get(ref(db, "serena_rules"));

        customSerenaRules = snapshot.exists() ? Object.values(snapshot.val()) : [];
    ...
    return customSerenaRules;

}

// ルールベースの応答生成（将来的に本物のAI APIへ差し替え可能な形にしている）
// 戻り値: { reply, link_url, link_label, choices }
async function serenaGetReply(userText) {

    const text = userText.toLowerCase();

    // 管理画面から追加されたルールを優先して照合し、無ければ既定のルールを見る
    const customRules = await loadCustomSerenaRules();
    const rules = [...customRules, ...DEFAULT_SERENA_RULES];
    ...
    for (const rule of rules) {
        if ((rule.keywords || []).some((k) => text.includes(String(k).toLowerCase()))) {
            return {
                ...
                choices: Array.isArray(rule.choices) ? rule.choices : []
            };
        }
    }

    return {
        reply: "申し訳ございません、まだそのご質問にはうまくお答えできません。今後のアップデートで、より賢く対応できるようにしていく予定です。",
        link_url: "",
        link_label: "",
        choices: []
    };
...
// セレナのUI（呼び出しボタン・チャットパネル）を初期化する
function initSerena(member) {

    if (document.getElementById("serenaButton")) return; // 二重生成防止

    // 応答ルールを先読みしておく(初回発話の遅延を減らすため)
    loadCustomSerenaRules();

    const button = document.createElement("button");
    button.id = "serenaButton";
    button.textContent = "🌸";
    button.title = "セレナを呼び出す";

    const panel = document.createElement("div");
    panel.id = "serenaPanel";
    panel.className = "serena-panel";

    panel.innerHTML = `
        <div class="serena-header">
            <span>🌸 セレナ</span>
            <button type="button" id="serenaCloseButton" class="serena-close">×</button>
        </div>
        <div class="serena-log" id="serenaLog"></div>
        <div class="serena-input-row">
            <input type="text" id="serenaInput" placeholder="セレナに話しかける...">
            <button type="button" id="serenaMicButton" title="音声入力">🎤</button>
            <button type="button" id="serenaSendButton">送信</button>
        </div>
    `;
    ...
    const input = panel.querySelector("#serenaInput");
    const sendButton = panel.querySelector("#serenaSendButton");
    const micButton = panel.querySelector("#serenaMicButton");
    const closeButton = panel.querySelector("#serenaCloseButton");
    ...
    button.onclick = () => {
        ...
        const bubble = document.getElementById("serenaBubble");
        if (bubble && show) bubble.classList.remove("active");

        if (show && !greeted) {

            greeted = true;
            const greeting = `こんにちは、${member.username}さん。何かお手伝いできることはありますか？`;
            serenaLogLine("serena", greeting);
            serenaSpeak(greeting);
    ...
    // ユーザーからの発話(文字入力・音声入力・選択肢クリックの共通処理)
    async function processUserMessage(text) {

        serenaLogLine("user", text);

        const result = await serenaGetReply(text);

        serenaLogLine("serena", result.reply, result);
        serenaSpeak(result.reply);
    ...
    // 選択肢ボタンがクリックされた時（serenaLogLineから発火される）
    document.addEventListener("serena:choice", (event) => {
        processUserMessage(event.detail);
    });

    // 音声入力（対応ブラウザのみ。主要ブラウザではChrome系のみ対応）
    const SpeechRecognitionCtor = window.SpeechRecognition || window.webkitSpeechRecognition;

    if (SpeechRecognitionCtor) {

        const recognition = new SpeechRecognitionCtor();
        recognition.lang = SERENA_VOICE_LANG;
        ...
        micButton.style.display = "none"; // 音声入力に対応していないブラウザでは非表示
