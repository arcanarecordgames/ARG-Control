...
function serenaLogLine(who, text) {
    ...
    line.textContent = (who === "serena" ? "🌸 " : "🧑 ") + text;
...
async function serenaGetReply(userText) {
    ...
    return "申し訳ございません、まだそのご質問にはうまくお答えできません。今後のアップデートで、より賢く対応できるようにしていく予定です。";
...
function initSerena(member) {
    ...
    async function handleSend() {
        ...
        const reply = await serenaGetReply(text);

        serenaLogLine("serena", reply);
        serenaSpeak(reply);
