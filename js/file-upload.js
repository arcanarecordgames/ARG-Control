...
uploadButton.onclick = () => {
    ...
    if (!file) {
        alert(tx("ファイルを選択してください"));
        return;
    }

    if (file.size > MAX_SIZE_BYTES) {
        alert(tx("ファイルサイズは50MBまでです"));
        return;
    }
    ...
    uploadTask.on(
        ...
        async () => {
            ...
            try {
                ...
                await pushNews("file", `新しいファイルが共有されました：${file.name}`, "file-detail.html?id=" + recordRef.key, null);
                await logSystemAction(member, "FILE_UPLOAD", file.name);

                alert(tx("アップロードが完了しました"));
