...
import { getLoggedInMember, attachLogoutButton, isLevel5, applyTheme } from "./common.js";
...
get(ref(db, "members/" + member.member_id + "/preferences"))
    .then((snapshot) => {
        ...
        styleSelect.value = prefs.style || localStorage.getItem("ARG_STYLE") || "terminal";
...
function previewTheme() {
    applyTheme(themeSelect.value, styleSelect.value);
}

themeSelect.onchange = previewTheme;
styleSelect.onchange = previewTheme;
...
saveDesignButton.onclick = async () => {
    ...
    try {
        ...
        await update(ref(db, "members/" + member.member_id + "/preferences"), { theme, style });
...
if (!isLevel5(member)) {
    ...
    purgeLogsButton.onclick = async () => {
        ...
        if (!confirm(`${retentionDays}日より古いログイン履歴を削除します。よろしいですか？`)) return;
        ...
        try {
            ...
            alert(`${deleteCount}件の古いログを削除しました`);
