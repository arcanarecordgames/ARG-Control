...
import { getLoggedInMember, initHeader, isLevel5, applyTheme, getLang, setLang, applyI18n, getAutoLogoutMinutes, setAutoLogoutMinutes, tx, getSerenaVoiceEnabled, setSerenaVoiceEnabled } from "./common.js";
...
const serenaVoiceCheckbox = document.getElementById("serenaVoiceCheckbox");
const saveSerenaVoiceButton = document.getElementById("saveSerenaVoiceButton");

serenaVoiceCheckbox.checked = getSerenaVoiceEnabled();

get(ref(db, "members/" + member.member_id + "/preferences/serena_voice_enabled"))
    .then((snapshot) => {
        if (snapshot.exists()) {
            serenaVoiceCheckbox.checked = snapshot.val();
            setSerenaVoiceEnabled(snapshot.val());
        }
    })
    .catch((error) => console.error(error));

saveSerenaVoiceButton.onclick = async () => {

    const enabled = serenaVoiceCheckbox.checked;

    setSerenaVoiceEnabled(enabled);

    try {
        await update(ref(db, "members/" + member.member_id + "/preferences"), { serena_voice_enabled: enabled });
    } catch (error) {
        console.error(error);
    }

    alert(enabled ? "セレナの音声ガイドをONにしました" : "セレナの音声ガイドをOFFにしました");
