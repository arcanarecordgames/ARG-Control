...
「セレナ応答管理」画面から、キーワード＋回答に加えて以下を設定できるようになりました。

## リンク
- 回答と一緒に、クリックできるリンクボタンを表示できます
- 「リンク」欄にはサイト内のページ（例：`online-time.html`）でも、外部URL（`https://...`）でもどちらも指定可能です
- 外部URL（http/httpsから始まるもの）は新しいタブで開き、サイト内のページは同じタブで移動します
- 「リンクの表示文字」は空欄でも構いません（その場合は「詳しくはこちら」と表示されます）

## 選択肢（利用者への質問）
- 回答の下に、複数のボタンを表示できます（「＋ 選択肢を追加」で追加）
- 各選択肢には「ボタンの表示文字」（例：はい）と「押した時にセレナへ送る内容」（例：はい）を設定します
- 利用者がボタンを押すと、それを実際に入力したのと同じ扱いでセレナに送られ、**その内容にマッチする別のルールがあれば続けて回答**します。これにより「はい/いいえ」のような簡単な分岐会話が組めます

## 実装メモ
- `serena_rules/{id}` に `link_url` / `link_label` / `choices`（`[{label, value}]`の配列）を追加保存
- `common.js` の `serenaGetReply()` の戻り値を文字列からオブジェクト（`{ reply, link_url, link_label, choices }`）に変更し、`serenaLogLine()` がリンクや選択肢ボタンも一緒に描画するようにしました
- 音声入力・文字入力・選択肢クリックのいずれから来たメッセージも、共通の `processUserMessage()` で処理するように整理しました

---

# 🆕 セレナの音声ガイド ON/OFF設定

SETTINGS画面の個人設定に「🌸 セレナの音声ガイドを有効にする」チェックボックスを追加しました。

- OFFにすると、イベント音声ガイド（ログイン挨拶・エラー通知・新着メール・タスク期限）やチャットでの返答が、**音声の読み上げだけ止まります**。チャット欄の文字での表示や、吹き出しでのお知らせはこれまで通り行われます
- アカウント（`members/{id}/preferences.serena_voice_enabled`）とこの端末（localStorage）の両方に保存され、他の端末でログインしても自動反映されます
- デフォルトはON（音声あり）です
